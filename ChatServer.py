...
class CRCServer(object):
    ...
    def setup_server_socket(self):
        ...
        server_socket.bind((self.connect_to_host_addr, self.port))
        server_socket.listen(1)
        server_socket.setblocking(False)
        events = selectors.EVENT_READ
        socket_data = ServerData(self.id, self.server_name, self.server_info)
        self.sel.register(server_socket, events, socket_data)
    ...
    def connect_to_server(self):
        ...
        sock.send(ServerRegistrationMessage.bytes(self.id, 0, self.server_name, self.server_info))
    ...
    def check_IO_devices_for_messages(self):
        ...
        while not self.request_terminate:
            # TODO: Implement the above functionality
            registered_IO_ready_to_process = self.sel.select(timeout=0.1)

            for registered_IO, event_mask in registered_IO_ready_to_process:
                sock_data = registered_IO.data

                if isinstance(sock_data, ServerData) and event_mask & selectors.EVENT_READ:
                    registered_IO.data.write_buffer += event_mask.to_bytes(1, 'big')
                    self.accept_new_connection(registered_IO)
                else:
                    self.handle_io_device_events(registered_IO, event_mask)
    ...
    def cleanup(self):
        ...
        for socket in sockets:
            self.sel.unregister(socket.fileobj)
            socket.fileobj.close()
    ...
    def accept_new_connection(self, select_key):
        # TODO: Implement the above functionality
        event_mask = int.from_bytes(select_key.data.write_buffer, 'big')
        select_key.data.write_buffer = b''
        if self.id == select_key.data.id:
            connection_socket, addr = select_key.fileobj.accept()
            connection_socket.setblocking(False)
            self.sel.register(connection_socket, selectors.EVENT_READ | selectors.EVENT_WRITE, ConnectionData())
        else:
            self.handle_io_device_events(select_key, event_mask)
    ...
    def handle_io_device_events(self, io_device, event_mask):
        ...
        if event_mask & selectors.EVENT_READ:
            response = io_device.fileobj.recv(2048)
            if response:
                self.handle_messages(io_device, response)
            else:
    ...
    # This function handles the initial registration process for new servers. Upon receiving a Server Registration message,
    ...
    def handle_server_registration_message(self, io_device, message):
        ...
        if message.source_id in self.hosts_db:
            self.send_message_to_unknown_io_device(io_device, StatusUpdateMessage.bytes(message.source_id, 0, 0x02, "A machine has already registered with ID {}".format(message.source_id)))
            return

        new_server_data = ServerData(message.source_id, message.server_name, message.server_info)
        events = selectors.EVENT_READ | selectors.EVENT_WRITE

        if message.last_hop_id == 0:
            new_server_data.first_link_id = new_server_data.id
            self.adjacent_server_ids.append(new_server_data.id)
            self.sel.modify(io_device.fileobj, events, new_server_data)
        else:
            new_server_data.first_link_id = message.last_hop_id

            if message.source_id == message.last_hop_id:
                self.sel.modify(io_device.fileobj, events, new_server_data)

        if new_server_data.first_link_id not in self.adjacent_server_ids:
            self.adjacent_server_ids.append(new_server_data.first_link_id)

        self.hosts_db[new_server_data.id] = new_server_data
        if message.last_hop_id == 0:
            self.send_message_to_host(new_server_data.id, ServerRegistrationMessage.bytes(self.id, self.id, self.server_name, self.server_info))
            for ID in self.hosts_db:
                if isinstance(self.hosts_db[ID], ServerData) and ID != new_server_data.id:
                    self.send_message_to_host(new_server_data.id, ServerRegistrationMessage.bytes(self.hosts_db[ID].id, self.id, self.hosts_db[ID].server_name, self.hosts_db[ID].server_info))
                elif isinstance(self.hosts_db[ID], ClientData):
                    self.send_message_to_host(new_server_data.id, ClientRegistrationMessage.bytes(self.hosts_db[ID].id, self.id, self.hosts_db[ID].client_name, self.hosts_db[ID].client_info))

        self.broadcast_message_to_servers(ServerRegistrationMessage.bytes(new_server_data.id, self.id, new_server_data.server_name, new_server_data.server_info), new_server_data.first_link_id)
    ...
    # This function handles the initial registration process for new client. Upon receiving a Client Registration message,
    # check to ensure that this ID does not yet exist in the network. If the ID does already exist, send a StatusUpdateMessage
    # with message code 0x02 and the message content "A machine has already registered with ID [X]", where [X] is replaced with 
    ...
    def handle_client_registration_message(self, select_key, message):
        # TODO: Implement the above functionality
        if message.source_id in self.hosts_db:
            self.send_message_to_unknown_io_device(select_key, StatusUpdateMessage.bytes(message.source_id, 0, 0x02, "Someone has already registered with ID {}".format(message.source_id)))
            return

        new_client_data = ClientData(message.source_id, message.client_name, message.client_info)
        events = selectors.EVENT_READ | selectors.EVENT_WRITE

        if message.last_hop_id == 0:
            new_client_data.first_link_id = new_client_data.id
            self.adjacent_user_ids.append(new_client_data.id)
            self.sel.modify(select_key.fileobj, events, new_client_data)
        else:
            new_client_data.first_link_id = message.last_hop_id

        self.hosts_db[new_client_data.id] = new_client_data

        if message.last_hop_id == 0:
            self.send_message_to_host(new_client_data.id, StatusUpdateMessage.bytes(self.id, new_client_data.id, 0x01, "Welcome to the Clemson Relay Chat network {}".format(new_client_data.client_name)))
            for ID in self.hosts_db:
                if isinstance(self.hosts_db[ID], ClientData) and ID != new_client_data.id:
                    self.send_message_to_host(new_client_data.id, ClientRegistrationMessage.bytes(self.hosts_db[ID].id, self.id, self.hosts_db[ID].client_name, self.hosts_db[ID].client_info))

        self.broadcast_message_to_servers(ClientRegistrationMessage.bytes(new_client_data.id, self.id, new_client_data.client_name, new_client_data.client_info), new_client_data.first_link_id)
        self.broadcast_message_to_adjacent_clients(ClientRegistrationMessage.bytes(new_client_data.id, self.id, new_client_data.client_name, new_client_data.client_info), new_client_data.first_link_id)
    ...
    def handle_status_message(self, select_key, message):
        ...
        if message.destination_id == self.id or message.destination_id == 0:
            ...
            select_key.fileobj.send(StatusUpdateMessage.bytes(message.source_id, message.destination_id, message.status_code, message.content))
            #self.send_message_to_host(message.destination_id, StatusUpdateMessage.bytes(message.source_id, message.destination_id, message.status_code, message.content))
    ...
    def handle_client_chat_message(self, select_key, message):
        ...
        if message.destination_id in self.hosts_db:
            self.send_message_to_host(self.hosts_db[message.destination_id].first_link_id, ClientChatMessage.bytes(message.source_id, message.destination_id, message.content))
        else:
            self.send_message_to_host(select_key.data.id, StatusUpdateMessage.bytes(self.id, select_key.data.id, 0x01, "Unknown ID {}".format(message.destination_id)))
    ...
    def handle_client_quit_message(self, io_device, message):
        # TODO: Implement the above functionality
        if message.source_id in self.hosts_db:
            self.broadcast_message_to_servers(ClientQuitMessage.bytes(message.source_id, message.content), io_device.data.id)
            self.broadcast_message_to_adjacent_clients(ClientQuitMessage.bytes(message.source_id, message.content), io_device.data.id)
